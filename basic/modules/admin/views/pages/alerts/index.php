...
<div class="pull-left">
    ...
    <h5>Здесь Вы можете напрямую отправить уведомление пользователям</h5>
</div>
<div class="pull-right" style="padding-top: 20px;">
    <a href="<?php echo Url::to(['pages/alerts/alert']);?>" class="btn btn-warning"><i class="fa fa-plus"></i> Создать уведомление</a>
</div>
...
<script>
    ...
    function loadPage(page, order, asc) {
        $.ajax({
            url: '<?php echo Url::to(['pages/alerts/alerts']);?>',
            type: 'POST',
            data: {
                'page': page,
                'order': order,
            ...
            success: function (data) {
                ...
                for(var i=0;i<data.items.length;++i) {
                    ...
                    $tr.find('td:eq(1)').append($('<a></a>').attr('href', '<?php echo Url::to(['pages/alerts/alert']);?>&id='+data.items[i].id).text(data.items[i].name)     );
                    $tr.find('td:eq(2)').text(data.items[i].alert_type);
                    $tr.find('td:eq(3)').text(data.items[i].post_date);
                    $tr.find('td:eq(4)').html($('<a class="btn btn-info btn-sm" title="Редактировать" href="<?php echo Url::to(['pages/alerts/alert']);?>&id='+data.items[i].id+'"><i class="fa fa-pencil"></i></a>&nbsp;<a href="javascript:void(0);" class="btn btn-danger remove-btn btn-sm" data-id="'+data.items[i].id+'"><i class="fa fa-trash"></i></a>'));
    ...
    (function () {
        ...
        $('#tout').on('click', '.remove-btn', function () {
            ...
            $.ajax({
                url: '<?php echo Url::to(['pages/alerts/remove']);?>',
                'type': 'POST',
                data: {
